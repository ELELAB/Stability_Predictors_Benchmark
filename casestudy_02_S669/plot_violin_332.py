...
list_of_balanced_df = glob.glob(path+"balanced_files_332/*.csv")
dfs_balanced = [pd.read_csv(file) for file in list_of_balanced_df]

list_of_random_df = glob.glob(path+"random_subset_files_332/*.csv")
...
df_combined.to_csv("PCCs_332.csv")
###########################################
## Make the plot
###########################################
...
plt.tight_layout()
plt.savefig("plots/violin_plot_332.pdf")
plt.show()
...
plt.ylabel("PPC value")
plt.tight_layout()
plt.savefig("plots/scatter_plot_332.pdf")
plt.show()
